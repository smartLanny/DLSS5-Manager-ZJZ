...
async function smokeHoYo(options = {}) {
  ...
  assert([...initialCore().options].map(row => row.value).join('|') === '0.4.7beta|0.5.1-beta-ui1' && !initialEditor.querySelector('[data-gp-detail="rollback"]'), 'the dedicated HoYo menu has current 0.4.7 and latest 0.5 only');
  ...
  const unavailableCurrent = gp.assessments['fixture-hoyo'].coreVersions.find(row => row.id === '0.5.1-beta-ui1');
  ...
  assert(initialCore().value === '0.5.1-beta-ui1' && !initialEditor.__gpController.hasDraft() && count('apply') === 0, 'a fresh Feeder install defaults to the ready latest Core without applying or creating a replacement draft');
  ...
  assert(core && [...core.options].some(row => row.value === '0.5.1-beta-ui1' && !row.disabled), 'verified Core candidate is selectable before first installation');
  core.value = '0.5.1-beta-ui1'; core.dispatchEvent(new Event('change', { bubbles: true }));
  click('prepare'); await until(() => button('apply'), 'preview dialog');
  assert(mock.calls.filter(row => row[0] === 'preview').at(-1)[3].version === '0.5.1-beta-ui1' && count('apply') === 0 && count('start') === 0, 'the first-install Core choice reaches the existing HoYo preview without installing or launching');
  ...
  click('close-plan'); assert(count('apply') === 0 && initialEditor.__gpController.getState().draft.version === '0.5.1-beta-ui1', 'preview cancel never installs and retains the selected Core'); mock.requiresAntiCheat = true; click('prepare'); await until(() => button('apply'), 'second preview'); click('apply');
  ...
  assert(count('start') === 0 && gp.assessments['fixture-hoyo'].game.addonVersion === '0.5.1-beta-ui1', 'explicit installation reads back the selected Core and never auto-launches');
