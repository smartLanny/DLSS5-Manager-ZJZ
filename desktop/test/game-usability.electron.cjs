...
function installScenario() {
  ...
  for (const value of Object.values(mock.assessments)) {
    value.coreVersions = [{ id: '0.4.7beta', label: '0.4.7beta', ready: true }, { id: '0.5.1-beta-ui1', label: '0.5.1', ready: true }, { id: '0.4.2', label: '0.4.2', ready: true }];
...
async function smoke() {
  ...
  check([...host('fixture').querySelector('.gp-install-section [data-gp-field="version"]').options].map(n => n.value).join('|') === '0.4.7beta|0.5.1-beta-ui1', 'main Core selector has the two supported choices');
  ...
  check(Boolean(corePicker()) && [...corePicker().options].some(row => row.value === '0.5.1-beta-ui1' && !row.disabled), 'latest Core is selectable for explicit compatibility preview');
  ...
  check([...corePicker().options].some(row => row.value === '0.5.1-beta-ui1' && !row.disabled), 'HoYo native input exposes the standard 0.5 switch in the shared page');
  change(hoyoHost(), 'route', 'version', '0.5.1-beta-ui1'); click(hoyoHost(), 'preview');
