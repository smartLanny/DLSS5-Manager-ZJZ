  ...
  function open(options) {
    ...
    function render() {
      ...
      card.innerHTML = `<div class="maintenance-heading"><div><h3>维护与恢复</h3><p>${esc(game.name)}</p></div><button class="button subtle" type="button" data-maintenance-action="close"${disabled}>关闭</button></div>
        ...
        ${mode === 'clean' && !pending ? '<p class="config-note">清理分两步：先撤销管理器的改动，再确认外部文件。文件移入备份，不会删除；游戏本体、存档和原生 DLSS 不受影响。</p>' : ''}
        ...
        <div class="maintenance-footer"><span>还原只撤销管理器的改动，不等于恢复游戏原版。</span><button class="button subtle" type="button" data-maintenance-action="feedback"${disabled}>保存反馈</button></div>`;
    }
