  ...
  function mount(host, manager, options = {}) {
    ...
    function uniformNrFields(nr, available) {
      ...
      return `<section class="gp-section"><div class="gp-section-title"><h3>NR 画面增强</h3>${check('Enabled', '开启')}</div>${available ? '' : '<p class="gp-caption">配置暂不可编辑，请检查安装或读取错误。</p>'}
        ...
        ${capable('ReconstructionMode') ? `<section class="gp-layer gp-reconstruction"><h4>清晰度与暗噪</h4><div class="gp-controls">${choice('ReconstructionMode', '低分辨率细节增强', [[0, '关闭'], [1, '均衡'], [2, '精细']])}</div><div class="gp-small-actions">${check('NearBlackChromaGuard', '去除暗噪')}</div><p class="gp-caption">细节增强：NR 分辨率较低时保住头发、衣物和人脸轮廓。均衡开销较低，精细更细但更耗性能。</p><p class="gp-caption">去除暗噪：减少人脸和暗部的黑灰、彩色杂点。两项默认关闭，效果请进游戏对照。</p></section>` : ''}
