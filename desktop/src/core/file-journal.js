// Adapted from rakanki911/DLSS5-Swapper ccb67f4 (MIT).
// Copyright upstream contributors; see vendor/DLSS5-Swapper/LICENSE.
// Downstream addition: preserve an unsafe automatic rollback for explicit recovery.
'use strict';
...
// Additive owner metadata for callers with stricter recovery contracts. This
// does not change capture/rollback behavior of existing unannotated callers.
async function setOwner(gameDir, owner) {
  const state = active.get(rootKey(gameDir));
  if (!state || !owner || typeof owner !== 'object' || Array.isArray(owner)) throw error('errBackupInvalid');
  const text = JSON.stringify(owner);
  if (Buffer.byteLength(text) > 128 * 1024 || typeof owner.product !== 'string' || !owner.product) throw error('errBackupInvalid');
  if (state.owner && (state.owner.product !== owner.product || state.owner.exe !== owner.exe)) throw error('errBackupInvalid');
  state.owner = JSON.parse(text);
  await atomicJson(pendingPath(gameDir), state);
}
...
async function transaction(gameDir, work, options = {}) {
  ...
  try {
    ...
    active.delete(rootKey(gameDir));
    // An owner may be unable to durably protect a newly appeared external
    // target. Keep the existing WAL for explicit recovery, never blindly erase it.
    if (cause && cause.preservePending === true)
      throw Object.assign(error('errBackendRecovery'), { cause, pendingPreserved: true });
    // A stricter owner can preserve its own atomic recovery contract even
    // when committing the transaction (removing its WAL) fails after work.
    try { await (typeof options.recoverOnError === 'function' ? options.recoverOnError(gameDir) : recover(gameDir)); }
    catch (recoveryError) { throw Object.assign(error('errBackendRecovery'), { cause, recoveryError }); }
    throw cause;
  }
}
module.exports = { safePath, capture, setOwner, transaction, recover, pendingPath, atomicJson };
