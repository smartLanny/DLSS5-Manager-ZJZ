...
function createLegacyRuntime(options = {}) {
  ...
  function pool() {
    ...
    if (!manifest) fail('LEGACY_PACKAGE_MISSING', '当前 Core 的旧 Feeder 配套未内置。可选择 ' + RECOMMENDED_CORE_LABEL + ' 使用新版 Feeder，或在游戏确有 DLSS 时使用原生路线；重复导入 DLSS5 模型也无法补齐旧 Feeder。', { file: 'legacy-runtime/manifest.json' });
