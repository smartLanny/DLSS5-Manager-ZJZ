...
const expected = Object.freeze({
  source: '6e642c29e7c4de422e5479354b273b11f0603d44783e6054657d3060de7d8a88',
  x86: 'f9b8d9b32411cf7118e0f00f66af7528db51a2742e815c797a0e00b8878dd221',
  x64: 'cb89471ab6c3a378be140d83d195d69de9e5238a37af1e88e704f72a56b2d731'
});
...
async function controlled(runtime, architecture, build) {
  const relativeFixture = architecture === 'x86' ? 'build/feeder-d3d9on12-unbound-switch-run-r1' : 'build/feeder-d3d9on12-unbound-switch-run-x64-r1';
  ...
  const probeBuildDirectory = path.join(APP, architecture === 'x86' ? 'build/feeder-d3d9on12-unbound-switch-probe-r1' : 'build/feeder-d3d9on12-unbound-switch-probe-x64-r1');
  ...
  assert.equal(probeBuild.sourceSha256, await fileDigest(path.join(APP, 'build/feeder-d3d9on12-unbound-switch-source-r1/probe.cpp')));
  ...
  const report = { api: 'dx9', architecture, loadingBackend: 'local', proxyEntry: 'd3d9', controlledRuntimeVerified: true,
    ...
    proof: { beforeResize: { type: 'game-received-host-completion', ...groups[0] }, afterResize: { type: 'game-received-host-completion', ...groups[1] },
      ...
      reenableHistoryVerified: true, resetWithPendingDepthVerified: true, configurationPreserved: true, samples: states.samples },
    components, evidence: { fixture: relativeFixture, completedAt: run.completedAt,
      ...
      buildReportAsset: 'dx9-r2-build' }, alpha: { readbackVerified: false, note: 'The full add-on switch/Reset fixture does not inspect NR alpha.' } };
...
async function promote() {
  ...
  assert.equal(runtime.lock.manifestFingerprint, 'b2b0f9aaae51315306874834193b019db79cdd3122e35a1cead1367672cf9519', 'Promote only the observed pre-R2 pool');
  const sourceDirectory = path.join(APP, 'build/feeder-d3d9on12-fix-source-r1');
  const buildDirectory = path.join(APP, 'build/feeder-d3d9on12-fix-provider-r2');
  ...
  const archive = path.join(APP, 'build/feeder-d3d9on12-r2-promotion');
  ...
  fs.copyFileSync(path.join(APP, 'build/feeder-d3d9on12-unbound-switch-source-r1/probe.cpp'), path.join(archive, 'source/probe.cpp'));
  ...
  for (const item of tested) {
    ...
    for (const file of ['run-result.json', 'feeder-d3d9on12-probe.json', 'nr-switch-report.json', 'feeder-d3d9on12-probe.log', 'ReShade.log']) fs.copyFileSync(path.join(item.directory, file), path.join(dir, file));
  ...
  for (const item of tested) {
    ...
    asset.provenance = { ...asset.provenance, build: 'd3d9on12-provider-fix-r2', sourceSha256: expected.source, source: sources, buildReportAsset: 'dx9-r2-build' };
  }
  ...
  const buildReport = { schema: 1, revision: 'd3d9on12-provider-fix-r2', upstreamCommit: manifest.upstream.commit,
    ...
    archive: 'build/feeder-d3d9on12-r2-promotion', previousPoolFingerprint: runtime.lock.manifestFingerprint,
  ...
  const buildReportSource = 'acceptance/dx9-r2-build.json'; write(resolveFile(pool, buildReportSource), buildReport);
  ...
  manifest.assets.push({ id: 'dx9-r2-build', source: buildReportSource, role: 'evidence', architecture: null, mutable: false,
