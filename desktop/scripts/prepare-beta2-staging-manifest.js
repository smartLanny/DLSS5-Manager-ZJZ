...
const MFG = Object.freeze({ bytes:601088, sha256:'64184bb370f223c3cabb359010a9a64e114cdae6b62d8b014a731a602af0a0da' });
...
function createManifest({ prior, resourcesRoot, selectedIds, payloadRoot, runtime40, runtime50, mfg }) {
  ...
  return {
    ...
    mfg: { file:path.resolve(mfg), ...MFG, url:'https://github.com/mavismmg/MFGAdaUnlock-RenoDx/releases/download/0.9/renodx-mfgunlock.addon64' },
    components,
    resources,
...
function parseArgs(argv) {
  ...
  for (let i=2;i<argv.length;i+=2) {
    const key = argv[i];
    if (!/^--(?:prior-report|resources-root|components|payload|runtime40|runtime50|mfg|output)$/.test(key) || !argv[i+1]) fail('参数不完整。');
    out[key.slice(2)] = argv[i+1];
  }
  for (const key of ['prior-report','resources-root','components','payload','runtime40','runtime50','mfg','output']) if (!out[key]) fail(`缺少 --${key}。`);
  return out;
}
if (require.main === module) {
  try {
    ...
    const manifest = createManifest({ prior, resourcesRoot:args['resources-root'], selectedIds:args.components.split(',').filter(Boolean),
      payloadRoot:args.payload, runtime40:args.runtime40, runtime50:args.runtime50, mfg:args.mfg });
