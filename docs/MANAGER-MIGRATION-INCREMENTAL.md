...
- `desktop/package.json` moves the Electron release identity to `0.5.0-beta.1` and exposes `stage`, `build:base`, `build:offline` and portable variants.
...
- `desktop/src/product/fg-mfgunlock-providers.json` holds the MFG provider pins; `fg-mfgunlock-resources.js` now loads that catalog, makes official 0.9 the default, accepts its addon-only resource set, and retains 0.7/0.6.1 as pinned fallbacks. The 0.7 source-build verifier keeps its own source hash.
...
No game installation, stable release publication or main-branch merge is performed. New runtime tests were stopped at the owner's request; subsequent work is implementation, compilation and candidate packaging.
